...
export default function EditGroupQuiz() {
  ...
  useEffect(() => {
    if (questionsData?.data && questionIds.length > 0) {
      const questionsMap = new Map(
        (questionsData.data as any[]).map((q: any) => [q._id, q])
      );
      setQuestionObjs(
        questionIds
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateQuizMutation.mutateAsync({
        id: quizId,
        body: payload,
      });
      ...
      toast({
        ...
        description: res?.message || "The quiz has been updated.",
      });
      ...
      toast({
        ...
        description:
          error?.message || "An error occurred while updating the quiz.",
        variant: "destructive",
      });
    }
  };

  if (isGroupLoading || isQuizLoading) {
    return (
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Only group admins can edit quizzes for this group.
          </p>
          <Button
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                >
                  Manage Questions ({questionIds.length})
                </Button>
              </CardHeader>
